...
The UI is a static GitHub Pages dashboard (`docs/`) that updates automatically:
- **Cheapest hours** / **most expensive hours**
- **Best 4-hour window** (lowest average across any 4 consecutive hours)
- Estimated savings for shifting a flexible load (kWh)
- Plots: tomorrow forecast, predicted vs actual history, feature importance
...
- **Daily**: runs Notebook 2 (ingest) and Notebook 4 (inference + plots), commits updated assets under:
  - `docs/PricesDashboard/assets/img/`
  - `docs/PricesDashboard/assets/data/forecast_summary.json`
- **Monthly**: runs Notebook 3 (training)
...
## Dashboard
GitHub Pages serves the site from `docs/`. The dashboard is image-based, but also reads a small JSON summary generated by Notebook 4:
- `docs/PricesDashboard/assets/data/forecast_summary.json`